...
class Neo4jService {
  ...
  // Get graph around an entity with variable depth – FIXED
  static async getGraph(entityId, depth = 2) {
    const session = getSession();
    try {
      // Single query that returns both nodes and edges
      ...
      if (result.records.length === 0) {
        // Fall back: return just the root node
  ...
  // Multi-hop query: find drugs targeting proteins in a pathway
